...
You already noticed that while we learn about NLP we also experience some new advanced elements of R language. You already know `%>%` pipe, but recently R community is using more and more a simpler (but just as powerful) syntax for a pipe `|>`. It is made of two symbols: a vertical line `|` (which depending on your keyboard could be on the very right-hand-side, or very left-hand-side) and the larger-than symbol `>`. 
...
pacman::p_load_gh("trinker/lexicon") #First load lexicon package from their source code on github with p_load_gh
...
Look carefully at the outputs of above code. Two sentences provide slightly different outputs of the named entity recognition. Why do you think it finds some names and locations and not others?
...
#Now we create a dataframe, first removing any columns with NULL as Location and then unnest the entries where there are multiple countries in a tweet.
...
Just before we finish this notebook, let's use the person recognition tool on a text of a novel:
...
ordered_names_pp
```
WOW! notice that this is much better. There is still improvement, since e.g.. name `Elizabeth` got recognised 435 times but it actually occurs in text 634 times, but it could be the case that we need to perform a few extra steps, like text cleanup. What is amazing here, is that we did not have to specify all characters ahead of time!
