...
Wouldn't it be nice if the library could do the splitting into words for us? Of course! Indeed, this is R, so obviously there is a function for that! Indeed there are two functions we can use here: `stem_words()` and `stem_strings()` comes from the `textstem` package.
...
Again, here we use the `textstem` package to produce lemmas. (This package will also do stemming). Just like with above examples there are two functions we'll use: one expects vector as words, and one vector of more complex strings (which it will split into words by itself).
...
freq.distribution.upos$key <-
  factor(freq.distribution.upos$key,
         levels = rev(freq.distribution.upos$key)) #could use it without rev but need reorder in the barchart later
       # levels = freq.distribution.upos$key) 
barchart(
  key ~ freq,
 #reorder(key,freq) ~ freq,
