...
We picked a freely available copy (via Gutenberg project) of "Pride and Prejudice" by English author Jane Austen from 1813.
...
```
Now go to the Environment > pride_prejudice and click the 'excel' icon on the right. What do you see? Search for the word "chapter". Do you see which chapters did not become BANANA?
...
We will learn more about entity recognition in the incoming badges, but could you 'anonymity' the book by replacing all names of characters with a tag like `[CHARACTER]` and maybe all places with `[LOCATION]`. If you do not know the book, here's a list of some [characters](https://en.wikipedia.org/wiki/Pride_and_Prejudice#Characters). In terms of locations you could start with "Longbourn estate", "Meryton", "Hertfordshire" and "England". But also you might chose to 'anonymise' something completely different. How about all gerund verbs like `...ing` as `[ACTION]` or days of the week?
...
```
Where would you use it in practice? E.g. to remove gendered titles form a document:
```{r}
...
```
or if you have a long list of characters, you could try something smart, like making your own regex on the fly:
```{r}
