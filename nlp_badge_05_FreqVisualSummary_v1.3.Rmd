...
You can also extend the usual set of `stopwords` which one always removes like "and", "or", "the", etc. And add there words that are in your data (eg. 'http' in this dataset), so that it gets removed at the same time that the `stopwords` are removed. See example below:
...
# mini-activity: out of curiosity, add a few more words to this exclusion list, to keep the first results even more relevant (eg. data is obviously about covid_19 so maybe covid words can be removed?) 
```
...
```{r wordcloud1}
set.seed(1234) # see note below about seeds. Run this a few times with this line, and a few more times with this line commented out. What changes?
...
**What is a seed? (Simplified 'Computer Science' explanation)**. When you generate a wordcloud, you leave a lot of things to random chance, like position of words. This means that each time you generate a wordcloud it will look a little different. But sometimes, for reproducibility, you want to stop randomness, and you can achieve that by giving the computer your own 'seed of randomness'. It is a very specific number, that will force the 'random' elements, to always be the same. For the curious: If you do not plant a concrete seed, the library will pick one for you (e.g. current date as a millisecond), so each time you generate a wordcloud it will be slightly different.
...
#Show the word co-occurrences: these are words happening immediately next to each other frequently:
bigram_graph
```
...
# lexical complexity (why is the middle sentence less complex?)
```
and then:
```{r }
...
# percentage stopwords: (in the whole set)
```
and then:
