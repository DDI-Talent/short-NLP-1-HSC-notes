...
1. Find some (100? 500? 1000?) most common words in the book (e.g. using badge 5 activity "Terms frequency". You might also want to `anti_join(stop_words)`)
2. Create two new columns in which you will store the given words similarity to word `love` and another one for similarity with word `family` (e.g. by Using answers answers to badge 7 activity with Glove files)
...
This is a large piece of data to investigate... and we just don't have time to read the book (just now, I mean. Do read it later, if it's your cup of Earl Grey tea!).
...
Treat this exercise as an excuse to get to know the tools and building up some reproducible/trustworthy coding habits (like testing). Good luck.
