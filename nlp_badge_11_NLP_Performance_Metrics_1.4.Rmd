...
**Note before we start about Rstudio setup**: your R studio might be setup to replace Tabs with double spaces. If that starts happening in your text files (especially .ann files where there is a bit difference between meaning of tabs and spaces)... you might see some weird behaviours. This setting can be disabled in RStudio > Project Options > Code Editing > disable it there.
...
- from our 80 guesses of TRUE, 70 were correctly guessed (were indeed TRUE) and 10 were not correctly guessed (they were actually meant to be FALSE).
- from our 120 guesses of FALSE, 100 were correctly guessed (were indeed FALSE) and 20 were not correctly guessed (they were actually meant to be TRUE).
...
In R there is a useful 'interpreted print' called `cat()` - it stands for Concatenate and Print. Concatenate is a fancy word for gluing many strings together.
...
- We'll create and use function that turns an .ann file into a dataframe. 
...
And now let's use that function twice. After running it, check out the values in both dataframes `gold_annotations` and `pred_annotations` in the Environment section of your RStudio.
