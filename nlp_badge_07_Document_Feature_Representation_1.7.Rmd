...
# to be sure which one you're using you'd call them as pryr::inspect or tm::inspect (you'll see this later)
```
...
The `tm` package is a popular choice for text mining in R as it provides easy-to-use tools to create Document Term Matrices (DTMs) and apply term frequency (TF) and term frequency-inverse document frequency (TF-IDF) weightings. DTMs hold our numerical representations of the document - our document vectors.
...
If you've been trying for a few minutes, but cannot find above options in the documentation, just a look at the answer: [Hint: Answer](./hints/hint_7_1.Rmd)
...
```{r}
#Load the corona csv to a dataframe
...
**If you see warnings at this point...** When working on our own computers (not Noteable) We have found sometimes there are deprecation warnings when using this package and incompatibilities so recently I tend to use the tidytext and tm more. That said there are some excellent blogs where people have used quanteda particularly around politics to analyse text data.

[Here is the quanteda vignette](https://cran.r-project.org/web/packages/quanteda/vignettes/quickstart.html) Feel free to look at some of the options when using the quanteda package.
...
2.  Convert our tibble with term frequencies it to a Document-Term Matrix (DTM) using `cast_dtm()`.
...
Note that operations below will take a lot of memory, for people using an environment with limited memory (like Posit Cloud) try to load just the first 100000 lines. (full file has almost 2 million lines).

```{r}
# Load GloVe embeddings (I find sometimes trying to read as a csv can cause memory issues depending on your set-up)
IF_LOW_MEMORY_LOAD_JUST_THIS_MANY_WORDS <- 100000 # to load everything, change this value to -1
...
Here's a more clearly written function which will calculate similarity of two words for us. Math is the same, but the 'interface' is better - we just give it two words and glove matrix, and the function does the magic for us, like this: `get_word_similarity("hospital", "clinic", glove_matrix)`
...
get_connected_words <- function(target_word, glove_words_matrix, how_many = 10){
  ...
  # here, there is no second word! instead we care for similarities of every word. 
	...
	# and here we will order those similarities and just grab the top few.
...
Let's recap a bit about **FUNCTIONS**. You build your own functions to EXTEND the R language, and add more functionality to it. For example R might not have a function `is_positive( some_number )` that will tell you if number is positive... so you can build it
...
But what if you wanted to apply a function many times, e.g. to each item in a collection?
...
Some ideas: - given list of words (e.g. sick, ill) get a list of co-appearing words which they have in common e.g. all 2 words share co-occurrence with "treated"
